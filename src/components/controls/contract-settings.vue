...
export default {
  ...
  methods: {
    async init () {
      ...
      let configStruct = abi.structs.find(
      ...
      for (let conf in currentConfig) {
        let item = configStruct.find(cs => cs.name === conf)
        item.value = currentConfig[conf]
        if (typeof item.value === 'object' && item.value !== null) {
          item.value = JSON.stringify(item.value)
        }
      }
      this.contract_config = configStruct
    },
    ...
    parseConfig () {
      ...
      let newConfig = clone.reduce((result, item) => {
        switch (item.type) {
          case 'bool':
            if (
              item.value === 0 ||
